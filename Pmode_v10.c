...
{
	int n;
	double m0,m1,mc,md,E0,E1,Ea,Eb,Ec,Ed,dE=1e12;
...
double e0;
{
	...
	double H1,H0,K,W,X,F,V,Dv,gamma,Dp,N,start,end;
	double DH11,DK1,DW1,DX1,H01,H02,K1,K2,x,X1,X2,Xp1,Xp2,W1,W2,V1,V2;
	double w,aR,bR,gR,hR,kR,n,Y1,Y2,Z,DZ,DDZ,VZ,Ar1,Ar2,Ai1,Ai2,ar,ai,AM,AMmin=1e10;
	int wpf,rpf,pfEnd,n4;
	...
	for(;p>power;r=r+dr)
	{
		if(wpf>49995)
		{
			temp_rhoc=ne;temp_M=0;temp_R=0;temp_freq=0;temp_dpt=0;
			return 1;
		}
		wpf++;
		...
		I=I/(f+2*I/r/r/r)/Gc2;
		I=m*sqrt(m/I)*Gc2;
		start=((-0.0047+0.133*I+0.575*I*I)/mR-1e-6)*1.8;
		end=2e-4;

		for(w=start;w<=end;w=w+0.01e-4)
		{
			e=rhofile[0];
			p=pfile[0];
			B=Bcor[0];
			...
			H0=H01+x*H02; K=K1+x*K2; W=W1+x*W2; V=V1+x*V2;
		...
		for(w=start;w<=end;w=w+0.001e-5)
		{
			e=rhofile[0];
			p=pfile[0];
			B=Bcor[0];
	...
	Omega1=Omega0+0.001e-5;
	Omega0=Omega0-0.001e-5;
	return 0;
}
...
int main()
{
	...
	printf("Advanced codes P-mode_v10 for Linux\n\n");
	printf("================================================\n\n");
	temp=fopen("temp_dir","w");
	fclose(temp);
	remove("temp_dir");
	system("dir EoS_lib >> temp_dir");
	if((temp=fopen("temp_dir","r"))==NULL){printf("No Equation of State files detected, automatically quit.");fclose(temp);return 0;
